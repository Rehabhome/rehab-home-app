...
exports.handler = async (event) => {
  ...
  const { to, paciente_nombre, tipo_informe, fecha, profesional_nombre, campos } = payload;
  if (!to) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Falta email del paciente' }) };
  }

  // Render campos del informe (array de {label, value})
  ...
  const html = `
    <div style="font-family:Calibri,'Segoe UI',sans-serif;max-width:640px;margin:0 auto;">
      ...
      <div style="background:#f5f9fb;padding:24px 28px;border:1px solid #e6edf0;border-top:none;border-radius:0 0 8px 8px;">
        ...
        <p style="color:#1f2d3d;margin-bottom:4px;">Se ha generado el siguiente documento clínico para su registro:</p>
        ...
        <p style="color:#64748b;font-size:12px;margin:0;">Para consultas, responda este correo o comuníquese con nosotros.<br>Equipo <strong>RehabHome</strong> · rehabhome.cl</p>
      </div>
    </div>`;
